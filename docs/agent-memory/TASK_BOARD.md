...
- harden long-window `debug trace full` performance now that per-step sink calls, repeated market-context serialization, repeated quiet-step decision payload construction, repeated repository-side `json.dumps`, empty risk-outcome payload churn, and duplicated active-step outcome scans have all been reduced; next likely hotspot is full-mode row volume itself or whether active-step payloads need a leaner representation
