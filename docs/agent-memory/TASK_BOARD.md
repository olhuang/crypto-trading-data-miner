...
- turn the new repo-local strategy ablation runner into a reusable research loop by adding more named presets, stronger KPI/behavior diagnostics, and clearer compare-set/report conventions on top of the first `breakout_exit_tightness` slice
...
- extended `/monitoring` Backtests launch status so async launches now show `job_id`, job state, worker-alive state, final `run_id`, and last error inline while the polling flow runs
- hardened async backtest run jobs so orphaned `queued/running` jobs with expired worker heartbeats now self-normalize to `stale` instead of looking healthy forever after a worker/thread loss
- added cooperative async backtest job cancellation with `POST /api/v1/backtests/run-jobs/{job_id}/cancel`, launch-panel cancel controls, persisted `cancel_requested` metadata, and `canceled` run finalization for partially started persisted runs
- fixed an async backtest performance regression where cancel-status polling had accidentally become a per-bar DB lookup inside `progress_callback`; cancel checks are now fast in-memory on every bar and DB-backed only at heartbeat cadence
- added a first repo-local strategy ablation runner at `scripts/run_backtest_ablation.py`, verified the `breakout_exit_tightness` preset over eight variants on a short `2025-03-15 -> 2025-03-21` smoke window, and emitted a ranked report at `tmp/backtest_ablation_smoke.json`
- enriched the ablation runner output with per-variant behavior counts plus `leaderboard` / grouped preset summaries, making the JSON report usable for quick cohort comparison before persisting a larger compare batch
