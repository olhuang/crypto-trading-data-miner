...
- a built-in scheduler bootstrap plus unified config toggles now exist so recurring refresh/remediation can be switched on from one repo-visible place during local/dev runs
...
- `docs/quality-integrity-ui-plan.md` now tracks how dataset-integrity validation and BTC backfill status should be surfaced inside the current `/monitoring` Quality workspace
...
- now also has a first run-level expected-vs-observed aggregate view through `GET /api/v1/backtests/runs/{run_id}/expected-vs-observed` and the internal `/monitoring` Investigate workspace
